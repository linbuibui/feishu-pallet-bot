...
CONFIG = {
    "feishu": {
        ...
        "callback_url": "https://webhook-jsocpcgbnh.cn-hangzhou.fcapp.run"
    },
    "bitable": {
    ...
    if is_search_request(text):
        ...
        try:
            ...
            for record in records:
                ...
                result_text += f"状态: {status_emoji} {status}\n"
                result_text += f"物料: {material}\n"
                result_text += f"库位: {location}\n"
                result_text += "---\n"
...
1️⃣ 查询托盘
   ...
   例如：显示物料名称含有V241101批JP-1366片的托盘号给我

2️⃣ 查看帮助
   输入：帮助
"""
    
    return """您好！我可以帮您查询托盘信息。
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/webhook"):
            ...
            if challenge:
                self.send_json_response({"challenge": challenge})
            else:
                self.send_json_response({"status": "ok"})
        elif self.path == "/":
            self.send_json_response({"status": "ok", "service": "飞书物料托盘管理机器人"})
        else:
    ...
    def do_POST(self):
        if self.path.startswith("/webhook"):
            ...
            if event_type == "url_verification":
                self.send_json_response({"challenge": data.get("challenge")})
                return
            
            # 如果没有有效数据，也返回成功
            if not data:
                self.send_json_response({"challenge": self.path})
                return
            ...
            self.send_json_response({"code": 0})
        else:
            self.send_response(404)
            self.end_headers()
